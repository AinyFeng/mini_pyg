...
2. 页面加载完毕
   ...
   2. 把数据 设置给data中的一个变量
3. onShow
    0. 会到了商品详情页面 第一次添加商品的时候 手动添加了属性
      1. num=1;
      2. checked = true;
    1. 获取缓存中的购物车数组
    2. 把购物车数据 填充到data中
...
Page({
  ...
  onShow() {
    ...
    只要 有一个回调函数返回了false 那么不在执行循环, 直接返回false
    空数组 调用 every, 返回值就是true
    由于下面有循环,所以不需要用到every方法循环 const allhecked = cart.length ? cart.every(v => v.checked): false;
     */
    // let allChecked = true;
    // // 1 总价格 总数量
    // let totalPrice = 0;
  ...
  async handleItemNumEdit(e) {
    ...
    let {cart} = this.data;
    // 3. 找到被点击的索引
    let index = cart.findIndex(v => v.goods_name=== name);
    // 4. 判断是否要执行删除
