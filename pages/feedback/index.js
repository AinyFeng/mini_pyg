...
3. 当用户点击 提交
    ...
    3. 验证通过 用户选择的图片 上传到专门的图片的服务器 返回图片外网的连接
        1. 遍历图片数组
        2. 挨个上传
        3. 自己再维护图片数组 存放 图片上传后外网的连接
    4. 文本域 和外网的图片的路径 一起提交到服务器   前端模拟 不会发送请求到后台 ...
    5. 清空当前页面
    6. 返回上一页
 */
...
Page({
  ...
  handleSubmit() {
    // 1. 获取文本域的内容 图片数组
    const { textareaValue, chooseImgs} = this.data;
    // 2. 合法性的验证
    ...
    if (chooseImgs.length != 0) {
      chooseImgs.forEach((v, i) => {
        wx.uploadFile({
          // 图片要上传到哪里
          url: 'htts://images.ac.cn/Home/Index/UploadAction/',
          // 被上传的文件的路径
          filePath: v,
